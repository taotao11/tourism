...
<div class="layui-container">
  <c:if test="${page == null}">
    <script  type="text/javascript">
        window.location.href = "${pageContext.request.contextPath}/jList?current=0&&type=1";
    </script >
  </c:if>
                ...
                <c:forEach items="${c}" var="x">
                  <li>
                    ...
                    <span class="fly-list-nums">
                      <a href="${pageContext.request.contextPath}/jindian/set?id=${x.jindian.id}&type=0">修改</a>
                      <a href="${pageContext.request.contextPath}/jindian/delete?id=${x.jindian.id}&type=0">删除</a>
                      <a href="${pageContext.request.contextPath}/detail/${x.jindian.id}">查看评论</a>
                    </span>
                  </li>
                </c:forEach>
              </ul>
              ...
              <div style="text-align: center">
                <div class="laypage-main">
                  <c:if test="${page.current != 1}">
                    <a href="${pageContext.request.contextPath}/jList?current=${ page.current - 1 }&&type=1" class="laypage-last" title="尾页">上一页</a>
                  </c:if>

                  <span class="laypage-curr">${page.current}</span>

                  <c:if test="${page.current < page.pages}">
                    <a href="${pageContext.request.contextPath}/jList?current=${ page.current + 1 }&&type=1" class="laypage-last" title="尾页">下一页</a>
                  </c:if>
                </div>
